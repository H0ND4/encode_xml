...
def main():
    ...
    for directory in ['train','eval']:
        ...
        if not os.path.exists('data/{}/annotations/xml'.format(directory)):
            # ここで自動作成させて，そのフォルダにxml, matフォルダを作るとバグらん
            os.makedirs('data/{}/annotations/xml'.format(directory))

        #### 下の形式に合うようにフォルダを構成する！ 結果がxmlフォルダに格納される
        ...
        #手動だめ  パスが// \\とかなってても行けるのでビビんな
        #MAT_FILES_PATH = 'D:/Desktop/deeptraining_hands-master/data/train/annotations/mat'
        #XML_FILES_PATH = 'D:/Desktop/deeptraining_hands-master/data/train/annotations/xml'
        #IMG_FILES_PATH = 'D:/Desktop/deeptraining_hands-master/data/train/images'
